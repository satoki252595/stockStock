...
祝日は扱わない。祝日カレンダー（`jpholiday` 等）を持ち込むと依存とメンテ対象が
増え、しかも祝日は年 16 日で「1 営業日分の余裕」に吸収できる規模である。
1 営業日の欠落は緑〜黄に収まり、2 営業日で黄、3 営業日で赤になる設計にして
祝日 1 日分を吸収させる（採らなかった案: 祝日カレンダー依存の追加）。
"""
