...
実フィクスチャ: JPX data_j.xls（上場銘柄一覧）。personal-only (§2.1) の
私的検証用フィクスチャ（詳細は tests/fixtures/convert/README.md）。
...
class TestXlsToCsv:
    def test_real_data_j_readable_and_large(self):
        """実 data_j.xls が読めて全上場銘柄規模（>3000行）であること。"""
        sheets = xls_to_csv(_load_bytes(), "data_j.xls")
        assert len(sheets) >= 1
...
class TestConvertArtifact:
    def test_xls_success(self, tmp_path):
        ...
        convert_artifact(artifact, "xls")
        assert artifact.convert_status is ConvertStatus.DONE
        # data_j.xls は1シート → 変換版は1ファイル（命名規則 §5.2）
