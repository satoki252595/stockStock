...
- Notionは大量時系列に不向き → 横断DBは「最新スナップショット」+「イベント行」のみ。**全銘柄横断のOHLCV履歴はParquet/CSV（⑥）で提供**
...
│   └── 各銘柄ページ配下 「株価テクニカル履歴」子DB … 1行=1営業日。8,000行で _2, _3…
...
**① 銘柄マスタ**: 銘柄名(title)、銘柄コード(text)、市場区分・33業種・17業種(select)、EDINETコード、上場状態、最終データ更新日、現行履歴DB ID / 履歴シャード番号 / 履歴行数（prices_daily 所有。master_sync は触らない）、②③④⑤へのrelation
...
**①配下 株価テクニカル履歴**: 基準日(title)、②と同じ数値列＋来歴。原本→⑤は single_property。キー=基準日。同日再実行は upsert
...
| `prices_daily` | 毎営業日19:30 | yfinance一括→テクニカル計算→②upsert＋①配下履歴子DBへ日次追記。stooqフォールバック。原本=一括取得単位のCSV/JSON+Parquet変換版 | B |
...
- Notion: 全銘柄日次更新(②upsert=銘柄ごと検索+更新2req)≒7,800req → 2.5req/sで約52分/日。履歴子DBは初回に create_database+①ポインタ更新、以降は日付キーの検索+create/update と新規行時の行数更新。初回は追加で約1万req（合計約2時間弱）。原本は一括取得単位のため数十req
